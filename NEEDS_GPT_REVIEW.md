...
- 房间设备台账检查：Ruff、严格 MyPy、39 项 Pytest 和 JavaScript 语法检查通过；2312 修正版完成桌面/移动视觉验收，现场确认设备新增、更新、删除与自动信号分层展示均通过。
...
用户随后确认同一时刻的真实状态是 2 台电脑和 1 台手机已连接 Wi-Fi、电视关闭，而电脑自动发现仅持续看到 MediaRenderer 服务。这证明本机模式既会漏掉已连接终端，也不能把媒体服务等同于电视开机。修正版将人工事实与自动信号分层：2312 现场确认台账记录 4 台设备及其状态；MediaRenderer 只显示为 60% 的“媒体服务端点”，并明确可能来自待机电视、机顶盒或电脑。正式八房间总览已经具备代码和演示闭环，但当前电脑没有八个目标 profile，不能将 2312 单房间结果扩展为八房间现场验收。
